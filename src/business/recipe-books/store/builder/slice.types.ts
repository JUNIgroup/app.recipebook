...
export type BucketsActionCreator<T extends BucketStructure> = {
   ...
   */
  upsertBuckets: <Payload = { documents: T['bucket'][]; deleted: ID[] }>(payload: Payload) => PayloadAction<Payload>
  ...
  upsertCollection: <
    ...
    Payload = { bucketId: ID; collectionName: CN; documents: T['collections'][CN][]; deleted: ID[] },
  >(
    payload: Payload,
